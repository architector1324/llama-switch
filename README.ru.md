...
Используйте плейсхолдеры `${HOST}`, `${PORT}` и `${CTX}` в строке команды запуска; приложение автоматически подставит нужные значения (порт по умолчанию `11435`, или назначенный приложением).
